...
const PROTECTACTINFO c_protectActInfo[32] = 
{
	...
	{"逆功率",		"REVERSE POWER",			PROTECTACT_NULL},
...
{
	if(pDataU->m_nData>=187 && pDataI->m_nData!=0)			//FixTo:173(=10V);
	{
...
{
	WORD wSetI = SETPOINT(wSD_SetI);
	WORD wDataI = s_wDataIMax;
	BOOL bLock = FALSE;
	if(SETPOINT(wSD_TogFX)==PROTECTTOG_ON) wDataI = s_wDataIMaxFX;
	if(SETPOINT(wSD_TogDY)==PROTECTTOG_ON && s_wDataUllMin>=SETPOINT(wSD_SetU)) bLock = TRUE;
...
{
	WORD wSetI = SETPOINT(wXSSD_SetI);
	WORD wDataI = s_wDataIMax;
	BOOL bLock = FALSE;
	if(SETPOINT(wXSSD_TogFX)==PROTECTTOG_ON) wDataI = s_wDataIMaxFX;
	if(SETPOINT(wXSSD_TogDY)==PROTECTTOG_ON && s_wDataUllMin>=SETPOINT(wXSSD_SetU)) bLock = TRUE;
...
{
	WORD wSetI = SETPOINT(wDSXGL_SetI);
	WORD wDataI = s_wDataIMax;
	BOOL bLock = FALSE;
	if(SETPOINT(wDSXGL_TogFX)==PROTECTTOG_ON) wDataI = s_wDataIMaxFX;
	if(SETPOINT(wDSXGL_TogDY)==PROTECTTOG_ON && s_wDataUllMin>=SETPOINT(wDSXGL_SetU)) bLock = TRUE;
...
{
	ProtectIDMT(PROTECT_FSXGL,SETPOINT(wFSXGL_IDMT),SETPOINT(wFSXGL_SetK),SETPOINT(wFSXGL_SetI),s_wDataIMax);
}
...
void CMyProtectItemHJS::Run(void)
{
	...
	{
		ProtectXGE(PROTECT_HJS,s_wDataIMax,SETPOINT(wHJS_SetI),SETPOINT(wHJS_SetT),FALSE);
	}
}
...
{
	ProtectXGE(PROTECT_GFH,s_wDataIMax,SETPOINT(wGFH_SetI),SETPOINT(wGFH_SetT),FALSE);
}
...
{
	ProtectXGE(PROTECT_FXDL,g_data3I2.m_nData,SETPOINT(wFXDL_SetI),SETPOINT(wFXDL_SetT),FALSE);
}
...
{
	ProtectXGE(PROTECT_LXDL,g_data3I0.m_nData,SETPOINT(wLXDL_SetI),SETPOINT(wLXDL_SetT),FALSE);
}
...
void CMyProtectItemGDY::Run(void)
{
	...
	ProtectXGE(PROTECT_GDY,s_wDataUllMax,SETPOINT(wGDY_SetU),SETPOINT(wGDY_SetT),bLock);
}
...
void CMyProtectItemDDY::Run(void)
{
	...
	ProtectXLE(PROTECT_DDY,s_wDataUllMin,SETPOINT(wDDY_SetU),SETPOINT(wDDY_SetT),bLock);
}
...
void CMyProtectItemSY::Run(void)
{
	...
	ProtectXLE(PROTECT_SY,s_wDataUllMax,SETPOINT(wSY_SetU),SETPOINT(wSY_SetT),bLock);
}
...
void CMyProtectItemFXDY::Run(void)
{
	BOOL bLock = FALSE;
	if(g_breaker.GetStatus()==BOSTATUS_OPENED||g_app.IsPTBreak()) bLock = TRUE;
	ProtectXGE(PROTECT_FXDY,g_data3U2.m_nData,SETPOINT(wFXDY_SetU),SETPOINT(wFXDY_SetT),bLock);
}
...
#endif
	ProtectXGE(PROTECT_LXDY,g_data3U0.m_nData,SETPOINT(wLXDY_SetU),SETPOINT(wLXDY_SetT),bLock);
}
...
void CMyProtectItemFHDYGL::Run(void)
{
	...
	{
		wLockDY = 1;
		if(s_wDataUllMin<SETPOINT(wFHDYGL_SetU)) wLockDY = 2;
	}
	if(SETPOINT(wFHDYGL_TogFXDY)==PROTECTTOG_ON)
	{
		wLockFX = 1;
		if(g_data3U2.m_nData>SETPOINT(wFHDYGL_SetUFX)) wLockFX = 2;
	}
	...
	ProtectXGE(PROTECT_FHDYGL,s_wDataIMax,SETPOINT(wFHDYGL_SetI),SETPOINT(wFHDYGL_SetT),(wLockDY||wLockFX));
}
...
void CMyProtectItemDZJZ::Run(void)
{
	...
	{
		if(!g_app.IsPTBreak()&&s_wDataUllMin<SETPOINT(wDZJZ_SetU)) bLock = TRUE;
	}
...
{
	WORD wSetURst = SETPOINT(wDYJL_SetU)+(SETPOINT(wDYJL_SetU)+10)/20;
	...
	ProtectXLE(PROTECT_DYJL,s_wDataUllMin,SETPOINT(wDYJL_SetU),SETPOINT(wDYJL_SetT),bLock);
}
	...
	if(g_breaker.GetStatus()==BOSTATUS_OPENED)
	{
		...
		if(m_wTime>=SETPOINT(wCHZ_SetT))
		{
			if(SETPOINT(wCHZ_TogWY)==PROTECTTOG_ON)
			{
				...
				if(nmaxUx<563||nmaxU<563) g_protect.SetProtectAct(PROTECT_CHZ,0xffff,0x5555);	//FixTo:563(=30V),900(=30V);
			}
...
{
	if((g_deviceInfo.wSwitchStatus&SS_IN09)==0 && IsLine2HavePower()==TRUE)
	{
		switch(m_wRunStep)
		{
		case RUNSTEP_CHARGING: RunCharging(); break;
...
void CMyProtectItemBZT::RunStartup(void)
{
	if(m_wTime)
	{
		...
		if(g_breaker.GetStatus()==BOSTATUS_OPENED)
		{
			if(SETPOINTCFG(PROTECT_BZT)==PROTECTCFG_TRP)
			{
...
BOOL CMyProtectItemBZT::IsLine1LostPower(void)
{
	...
	if(s_wDataIMax==0 && s_wDataUllMax<SETPOINT(wBZT_SetU)) return TRUE;
	return FALSE;
}
...
{
	if(SETPOINT(wGR_SetI)==0) return;
	WORD wDataI1 = CMyMeasure::CalculateNegSeq(&g_dataIC,NULL,&g_dataIA)/3;
	...
	DWORD dwSetI2 = (DWORD)SETPOINT(wGR_SetI)*(DWORD)SETPOINT(wGR_SetI);
	dwSetI2 >>= 2;
	dwDataIeq2 >>= 2;
...
void CMyProtectItemNGL::Run(void)
{
	...
	if(lPa+lPb+lPc<lNGL_SetP)
	{
		...
		if(m_wTime>=SETPOINT(wNGL_SetT))
		{
			...
			g_protect.SetProtectAct(PROTECT_NGL,0xffff,0x3333);
		}
	}
...
void CMyProtectItemQDSJGC::Run(void)
{
	...
	else ProtectXGE(PROTECT_QDSJGC,s_wDataIMax,(WORD)(SETPOINT(wMotor_SetI)*6/5),3,FALSE);
}
...
{
	ProtectXGE(PROTECT_IXDSX,g_dataIx.m_nData,SETPOINT(wIXDSX_SetI),SETPOINT(wIXDSX_SetT),FALSE);
}
...
{
	ProtectIDMT(PROTECT_IXFSX,SETPOINT(wIXFSX_IDMT),SETPOINT(wIXFSX_SetK),SETPOINT(wIXFSX_SetI),g_dataIx.m_nData);
}
...
void CMyProtectiBCUX42::iiiRun(void)
{
	...
	{
		WORD wSetIStart0_1 = SETPOINT(wMotor_SetI)/10;
		...
		WORD wSetIStart1_2 = SETPOINT(wMotor_SetI)*6/5;
