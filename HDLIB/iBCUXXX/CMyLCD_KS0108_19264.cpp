...
void CMyLCD_KS0108::LCDWhile(BYTE BlockSel)
{
	...
	if(BlockSel==0) while((IOLCD_C3&0x90)&&(wCountDelay--));
	else if(BlockSel==2) while((IOLCD_C2&0x90)&&(wCountDelay--));
	else if(BlockSel==1) while((IOLCD_C1&0x90)&&(wCountDelay--));
}

void CMyLCD_KS0108::LCDSndData(BYTE BlockSel,BYTE Data)
{
	LCDWhile(BlockSel);
	if(BlockSel==0) IOLCD_D3 = Data;
	else if(BlockSel==2) IOLCD_D2 = Data;
	else if(BlockSel==1) IOLCD_D1 = Data;
}

void CMyLCD_KS0108::LCDSndCommand(BYTE BlockSel,BYTE Command)
{
	LCDWhile(BlockSel);
	if(BlockSel==0) IOLCD_C3 = Command;
	else if(BlockSel==2) IOLCD_C2 = Command;
	else if(BlockSel==1) IOLCD_C1 = Command;
}
...
void CMyLCD_KS0108::iiiFlush(void)
{
	...
	if(m_bOpened)
	{
		...
		LCDSndCommand(2,0x3f);
	}
	else
	{
		LCDSndCommand(0,0x3e);
		LCDSndCommand(1,0x3e);
		LCDSndCommand(2,0x3e);
	}
