...
static MYCOMM** s_ppComm;

//-----------------------------------------------------------------------------
void COMM_SetComm(void** ppComm)
{
	s_ppComm = (MYCOMM**)ppComm;
}
...
void COMM_OnPolling(void)
{
	...
	case COMMSTATUS_SNDING1:
		...
		s_wTXLength = i;
		(*s_ppComm)->ClearCom(*s_ppComm);
		wResult = 0;
		for(i=0;i<20;i++)
		{
			wResult += (WORD)(*s_ppComm)->WriteCom(*s_ppComm,s_TXBuffer+wResult,s_wTXLength-wResult);
...
void COMM_OnTimer10ms(void)
{
	...
	if(s_wRcvFlag>=10)
	{
		...
		{
			WORD wTemp = (WORD)(*s_ppComm)->ReadCom(*s_ppComm,s_RXBuffer+s_wRXLength,COMM_BUFFERSIZE-s_wRXLength);
			if(wTemp) 
				s_wRXLength += wTemp;
		}
	}
